...
// creating a struct for signals we need to be carried forward
...
PC ProgramCounter(.CLK(CLK), .RST(RST), .PC_WRITE(pcWrite),.PC_SOURCE(pcSource), .JALR(jalr_if),.BRANCH (branch_if), .JAL (jal_if), .PC_OUT (pc_if));

logic [31:0] pc_de;
logic ld_use_hz, cntrl_haz, hold_cntrl_haz, control_haz_uncond;
...
CU_DCDR Decoder(
    .IR_FUNCT(decode_t.mem_type),
    .IR_30(ir_de[30]),
    .IR_OPCODE (decode_t.opcode),
    ...
    .MEM_WE2 (decode_t.memWrite),
    .MEM_RDEN2 (decode_t.memRead2));
...
assign decode_t.rs2_used = decode_t.rs2_addr != 0 
...
HAZARD_DETECTION HAZARD_DETECTION (
    ...
    .ex_rsl_used (execute_t.rs1_used),
...
TwoMux SRCA(
    .ALU_SRC_A(execute_t.alu_srcA),
    .RS1(frs1_ex),
    .U_TYPE(imm_ex.utype),
    .SRC_A(srcA)) ;

FourMux SRCB (
    .SEL (execute_t.alu_srcB),
    .ZERO (frs2_ex),
    .ONE (imm_ex.itype),
    .TWO (imm_ex.stype),
    .THREE (execute_t.pc),
